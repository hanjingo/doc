...
*A.HOME查询EXAMPLE.COM的典型递归DNS查询过程涉及多达10条消息。本地递归服务器（此处为GW.HOME）使用由ISP提供的DNS服务器。该服务器依次使用互联网根域名服务器和gTLD服务器（用于COM和NET TLD）来查找EXAMPLE.COM域名的名称服务器。该名称服务器（此处为A.IANA-SERVERS.NET）提供主机EXAMPLE.COM对应的IP地址。所有的递归服务器缓存学习到的任何信息供以后使用*
...
*DNS消息格式有一个固定的12字节头部。整个消息通常在UDP/IPv4数据报中运载，并且限于512字节。DNS UPDATE（动态更新DNS）使用字段名ZOCOUNT，PRCOUNT，UPCOUNT和ADCOUNT。特殊的扩展格式（称为ENDS0）允许消息大于512字节，DNSSEC需要这种消息*
...
*DNS消息中的查询（或问题）字段不包含TTL，因为它是不被缓存的*
...
*DNS资源记录的格式。对于互联网中的DNS，类（class）字段总是包含值1。TTL字段给出RR可以缓存的最大时间量（秒）*
...
*一个简单的DNS查询/相应的示例。本地DNS服务器（GW.HOME）向客户端（A.HOME）提供递归查询，并在请求的数量不在缓存中时使用ISP提供的DNS服务器*
...
- 动态委托发现系统（DDDS）
  ...
  *DDDS算法的抽象操作。允许非终结记录行成循环。每次迭代器都涉及应用唯一字符串的重写操作*
...
*DNS区域传输在服务器之间复制内容。一个可选的通知可以引起从服务器请求完整或增量区域传输*
