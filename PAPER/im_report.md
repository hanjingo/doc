...
1. 可靠性
   ...
   消息的可靠性是IM系统的核心功能之一，可靠性包括两个方面：

   - 消息不丢失

     对于离线消息要做存储，客户端发送完消息之后要确认(QOS)；

   - 消息不重复

     消息发送失败时需要做消息重发，为了防止消息重复，需要给每一个消息做一个唯一ID；

2. 低延迟

   低延迟的消息送达是IM系统的另一个核心功能，高延迟的IM系统等于不可用；延迟高会造成用户体验不佳，从而导致用户的大量流失。

3. 信息安全

   信息在传输过程中需要防止消息内容抓包破译，当用户用同一个账号在不同设备上进行多点登录时，要对设备做鉴权。
...
4. 信息存储
   ...
   - 数据合法合规的问题：根据相关法律法规的规定，国内的im系统需要接入公安部门的监控系统，要做好配合任务；有些地区的法律规定，当地的聊天信息必须要保存在当地，需要在当地建立机房节点。
...
2. 对于可靠性要求较高的场景，比如：交易，文件传输，建议使用TCP；
...
市面上几乎所有主流IM APP都是是使用私有协议，开发im系统时建议使用自定义消息协议。
...
- 序列化/饭序列化效率
  ...
  对于高并发的服务器来说，序列化/反序列化效率过于低下的编码方式，往往会带来CPU资源的大量占用，严重影响服务器的承载能力。
...
自定义序列化协议需要自己实现序列化与反序列化操作；

优点：

- 自由度高；
- 与业务联系紧密，剔除了通用方案的无关部分；
...
MessagePack是一种数据格式与JSON极为类似的编码方式；在存储时对数字，多字节字符，数组等都做了很多优化，减少了无用的字符，二进制格式，也保证不用字符化带来额外的存储空间的增加。
