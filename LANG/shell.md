...
| 命令  | 说明                    | 选项                                                         |
| ----- | ----------------------- | ------------------------------------------------------------ |
| tar   | `压缩/解压文件或文件夹` | - `c` 建立新的备份<br>- `z` 以gzip命令处理文件<br>- `x` 从备份文件中还原文件<br>- `v` 显示执行的过程<br>- `f` 指定备份文件<br>- `t` 列出备份文件的内容 |
| zip   | 压缩成zip文件           | - `q` 表现时指令执行过程<br>- `r` 递归处理<br>- `d` 从压缩文件内删除指定的文件<br>- `v` 显示指令执行过程 |
| unzip | 解压缩zip文件           | - `l` 显示压缩文件内所包含的文件<br>- `d` 解压到指定目录     |
| rar   | 压缩成rar文件           |                                                              |
| urar  | 解压缩rar文件           |                                                              |

```sh
# 示例
tar -czvf 1.tar.gz 1.txt # 将1.txt压缩为1.tar.gz
tar -tzvf 1.tar.gz # 列出压缩文件1.tar.gz的内容
tar -xzvf 1.tar.gz # 解压文件1.tar.gz
tar -xvf 1.tar.xz # 解压文件1.tar.xz

zip -qr test.zip ~/test # 将test目录压缩成test.zip文件
zip -dv test.zip /home/he/test/1.txt # 删除test.zip文件中的1.txt文件

unzip -l test.zip # 查看压缩文件`test.zip`中包含的文件
unzip test.zip -d ~/tmp #将文件`test.zip`解压到目录`~/tmp`
```
...
| ps     | 查看进程快照信息 | - `a` 显示同一终端下的所有程序<br>- `A` 显示所有进程<br>- `e` 等同于"-A"<br>- `f` 显示程序间的关系<br>- `u` 指定用户的所有进程<br>- `H` 显示树状结构<br>- `au` 显示较为详细的资讯<br>- `aux` 显示所有包含其它使用者的行程<br>**二级选项:**<br/>-- `lines 行数` 每页显示的行数<br/>-- `width 字符数`  每页显示的字符数 |
| top    | 查看进程持续信息 | - `c` 切换显示模式<br>- `q` 无延迟显示<br>- `i` 不显示闲置和僵尸进程<br>- `n` 设置更新次数<br>- `d` 设置更新时间<br>- `p` 指定进程id<br><br>**行内命令:**<br/>`空格键` 立即刷新<br/>`P` 以占据CPU百分比排序<br/>`M` 以占据内存百分比排序<br/>`T` 以累积占用CPU时间排序<br/>`t` 显示或隐藏进程和cpu状态信息<br/>`s <秒数>` 修改刷新时间间隔<br/>`k <进程id> <enter> <信号>` 杀死指定id的进程 |
| nohup  | 后台运行程序     |                                                              |
| screen | `分屏/共享屏`    | - `A` 将所有的视窗都调整为目前终端机的大小<br>- `d` 将指定的screen作业离线<br>- `h` 指定视窗缓冲区行数<br>- `r` 恢复指定的离线screen作业<br>- `x` 恢复之前离线的screen作业<br>- `S` 指定screen作业名称<br>- `ls` 显示目前所有的screen作业 |
| kill   | 杀进程           | - `9                                                         |

```sh
lsof | wc -l # 查看当前系统打开的文件数量
lsof -p xx | wc -l # 查看当前进程xx打开的文件数量

ps -aux | grep he # 查看用户he的所有进程]
ps -ef # 显示所有进程信息，包括进程的命令行
ps -axjf # 列出程序树信息
ps -aux|egrep '(vim|chrome)' # 列出与vim或chrome相关的进程

top -d 2 # 2秒刷新一次信息
top -p 2444 # 只显示进程2444的信息
#杀死id为2444的进程
top
>> k 2444
>> 15

nohup chrome & # 后台运行chrome并在当前目录生成一个默认名为nohup.out的日志文件
nohup chrome &>/dev/null & # 后运行chrome，同时不生成日志文件
nohup chrome >test.log 2>&1 & # 后台运行chrome，同时输出到日志文件test.log

screen # 创建screen作业
screen -ls # 列出所有的作业
screen -x he # 恢复一个名叫he的screen作业
# 创建一个名叫he的screen作业
screen -S he 
   <Ctrl+a+d> # 离开当前screen作业(注意：此作业会继续保留)
   exit # 直接退出当前作业，不保留
   
kill -9 1234 # 使用最高等级杀id为1234的进程i
```
...
| 命令      | 说明                             | 选项                                                         |
| :-------- | :------------------------------- | :----------------------------------------------------------- |
| ldd       | 查看程序的依赖共享库列表         | - `v` 详细模式<br>- `u`  打印未使用的依赖<br>- `d`  执行重定位和报告任何丢失的对象<br>- `r`  执行数据对象和函数的重定位，并且报告任何丢失的对象和函数 |
| which     | 查看命令的可执行文件路径         | - `n <数字>` 指定文件名最小长度<br>- `p+数字` 指定文件完全路径最小长度<br>- `w`  指定输出时栏位宽度<br>- `V`  显示版本信息 |
| tail      | 查看日志                         | - `f` TODO                                                   |
| file      | 查看文件类型                     | - `b`  不显示文件名<br>- `c` 显示指令执行过程<br>- `f` 指定文件名<br>- `L` 直接显示符号链接所指向的文件类型<br>- `m` 指定魔法数字文件<br>- `v` 显示版本信息<br>- `z` 解读压缩文件的信息 |
| nm        | 查看文件的符号信息               | - `a`  显示所有符号<br>- `D`  显示动态符号(对动态链接库有效)<br>- `g`  只显示外部符号<br>- `l`  使用debug信息找到文件名和行号<br>- `n`  按符号对应地址的顺序排序<br>- `p`  按照目标文件中遇到的符号顺序显示，不排序<br>- `P`  按照POSIX2.0标准格式输出<br>- `r`  反转排序<br>- `s`  当列出库中成员的符号时，包含索引<br>- `u`  只显示未定义符号<br><br>**二级选项**<br>`--defined-only` 只显示定义的符号<br/>`--no-demangle` 默认选项，不将低级符号名解析成用户级名字 |
| strings   |                                  |                                                              |
| size      | 查看文件各段和总和大小           |                                                              |
...
| objdump   | 反汇编程序                       | - `c`<br>- `x`                                               |
| addr2line | 翻译出错信息，定位出错的代码位置 | - `e`                                                        |
