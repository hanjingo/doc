...
* [算法](#算法)
    - [排序](#排序)
        + [1 快排算法及其时间复杂度分析](#1-快排算法及其时间复杂度分析)
    - [树](#树)
        + [1 B树](#1-b树)
        + [2 B+树](#2-b树)
        + [3 求二叉树最远叶子结点的距离](#3-求二叉树最远叶子结点的距离)
        + [4 层级遍历二叉树并计算每一层节点之和](#4-层级遍历二叉树并计算每一层节点之和)
        + [5 最小生成树](#5-最小生成树)
    - [链表](#链表)
        + [1 如何判断链表是否有环5th](#1-如何判断链表是否有环5th)
        + [2 如何逆转链表](#2-如何逆转链表)
    - [其他](#其他)
        + [1 LRU缓存实现](#1-lru缓存实现)
* [系统](#系统)
        + [1 进程，线程和协程以及它们的区别](#1-进程线程和协程以及它们的区别)
        + [2 可执行程序的结构](#2-可执行程序的结构)
        + [3 不同线程之间是否可以共享套接字](#3-不同线程之间是否可以共享套接字)
        + [4 如何提升memcpy的效率](#4-如何提升memcpy的效率)
        + [5 进程状态图](#5-进程状态图)
        + [6 多线程不安全的根本原因](#6-多线程不安全的根本原因)
        + [7 线程调度机制](#7-线程调度机制)
        + [8 生产与消费者模式下的线程安全](#8-生产与消费者模式下的线程安全)
        + [9 单例模式下的线程安全](#9-单例模式下的线程安全)
        + [10 I/O调优](#10-io调优)
* [分布式系统](#分布式系统)
        + [1 描述Raft协议和Paxos协议，以及它们的区别](#1-描述raft协议和paxos协议以及它们的区别)
        + [2 如何实现分布式锁](#2-如何实现分布式锁)
* [网络](#网络)
    - [TCP](#tcp)
...
* [C/C++](#cc)
    - [基本语法](#基本语法)
        + [1 指针和数组](#1-指针和数组)
        + [2 指针和引用](#2-指针和引用)
        + [3 C语言参数压栈顺序](#3-c语言参数压栈顺序)
        + [4 重载和覆盖](#4-重载和覆盖)
        + [5 C++中的四种`cast`](#5-c中的四种cast)
        + [6 static](#6-static)
        + [7 const](#7-const)
        + [8 空指针和悬垂指针的区别](#8-空指针和悬垂指针的区别)
        + [9 inline函数](#9-inline函数)
    - [面向对象](#面向对象)
        + [1 实现运行时多态有哪些方式](#1-实现运行时多态有哪些方式)
        + [2 为什么要用`virtual destructor`？为什么没有`virtual constructor`?](#2-为什么要用virtual-destructor为什么没有virtual-constructor)
        + [3 哪些函数不能是虚函数？](#3-哪些函数不能是虚函数)
        + [4 虚函数和纯虚函数的区别](#4-虚函数和纯虚函数的区别)
        + [5 构造函数和析构函数可以调用虚函数吗？](#5-构造函数和析构函数可以调用虚函数吗)
        + [6 什么是私有继承](#6-什么是私有继承)
        + [7 C++中空类默认产生哪些成员函数/变量](#7-c中空类默认产生哪些成员函数变量)
        + [8 explicit构造函数的作用](#8-explicit构造函数的作用)
        + [9 虚析构函数的作用](#9-虚析构函数的作用)
        + [10 复制构造函数的作用，与赋值函数的区别是什么](#10-复制构造函数的作用与赋值函数的区别是什么)
        + [11 抽象基类与纯虚函数的作用](#11-抽象基类与纯虚函数的作用)
        + [12 const与#define的区别](#12-const与define的区别)
        + [13 虚函数可以是内联函数吗](#13-虚函数可以是内联函数吗)
    - [STL](#stl)
        + [1 STL的allocator](#1-stl的allocator)
        + [2 `std::move`](#2-stdmove)
        + [3 迭代器失效怎么解决](#3-迭代器失效怎么解决)
        + [4 如何在两个线程之间共享套接字](#4-如何在两个线程之间共享套接字)
        + [5 智能指针的内存泄漏如何解决](#5-智能指针的内存泄漏如何解决)
        + [6 STL中map的实现](#6-stl中map的实现)
        + [7 STL的内存优化策略](#7-stl的内存优化策略)
        + [8 std::vector存储指针的情况](#8-stdvector存储指针的情况)
    - [编译原理](#编译原理)
        + [1 `malloc/free`，`new/delete`的区别](#1-mallocfreenewdelete的区别)
        + [2 字节对齐的规则](#2-字节对齐的规则)
        + [3 sizeof小结](#3-sizeof小结)
        + [4 `#pragma pack(x)`的作用](#4-pragma-packx的作用)
        + [5 extern "C"的作用](#5-extern-c的作用)
    - [模版](#模版)
    - [多线程](#多线程)
        + [1C++实现多线程的几种方式](#1c实现多线程的几种方式)
    - [SDK](#sdk)
        + [1如何保证SDK的类导出时，修改类结构不影响调用者](#1如何保证sdk的类导出时修改类结构不影响调用者)
* [Lua](#lua)
        + [Lua的优点](#lua的优点)
        + [Lua数据类型](#lua数据类型)
        + [谈谈Lua的GC](#谈谈lua的gc)
...
* [GO](#go)
        + [1 GO的不足](#1-go的不足)
        + [2 GO的协程原理](#2-go的协程原理)
        + [3 Go的GMP模型](#3-go的gmp模型)
        + [4 Select机制](#4-select机制)
        + [5 GC优化与内存逃逸](#5-gc优化与内存逃逸)
* [Skynet](#skynet)
...
* [Redis](#redis)
    - [使用](#使用)
        + [1 Redis为什么是单线程的](#1-redis为什么是单线程的)
        + [2 Redis的性能为什么这么高](#2-redis的性能为什么这么高)
        + [3 客户端是怎么接入Redis哨兵系统的](#3-客户端是怎么接入redis哨兵系统的)
    - [实现](#实现)
        + [1 为什么Redis用跳表不用RBtree](#1-为什么redis用跳表不用rbtree)
        + [2 Redis如何实现延时队列](#2-redis如何实现延时队列)
        + [3 Redis持久化](#3-redis持久化)
        + [4 Redis线程模型](#4-redis线程模型)
        + [5 Redis的过期机制](#5-redis的过期机制)
        + [6 谈谈Redis的布隆过滤器](#6-谈谈redis的布隆过滤器)
        + [7 谈谈Redis的持久化机制](#7-谈谈redis的持久化机制)
        + [8 Redis的主从同步是怎么实现的](#8-redis的主从同步是怎么实现的)
        + [9 Redis哨兵的实现](#9-redis哨兵的实现)
        + [10 Redis哨兵系统是怎么实现自动故障转移的](#10-redis哨兵系统是怎么实现自动故障转移的)
        + [11 Redis集群是怎么实现数据分片的](#11-redis集群是怎么实现数据分片的)
        + [12 Redis集群是怎么做故障转移和发现的](#12-redis集群是怎么做故障转移和发现的)
* [Mysql](#mysql)
    - [使用](#使用-1)
        + [1 Mysql有哪些优化措施](#1-mysql有哪些优化措施)
        + [2 说说数据库索引的使用及其原理](#2-说说数据库索引的使用及其原理)
        + [3 mysql事物](#3-mysql事物)
        + [4 脏页是什么](#4-脏页是什么)
        + [5 Mysql的隔离级别是怎么样的](#5-mysql的隔离级别是怎么样的)
        + [6 Mysql中由哪些锁](#6-mysql中由哪些锁)
        + [7 Mysql如何解决幻读问题](#7-mysql如何解决幻读问题)
        + [8 主键，唯一性索引，普通索引的区别是什么](#8-主键唯一性索引普通索引的区别是什么)
        + [9 什么是分表分库](#9-什么是分表分库)
        + [10 如何实现跨库分页查询](#10-如何实现跨库分页查询)
        + [11 Checkpoint是什么](#11-checkpoint是什么)
        + [12 undo log, redo log, bin log是什么](#12-undo-log-redo-log-bin-log是什么)
        + [13 存储过程的使用](#13-存储过程的使用)
    - [实现](#实现-1)
        + [1 InnoDb的底层数据结构](#1-innodb的底层数据结构)
        + [2 磁盘结构分析与数据存储原理](#2-磁盘结构分析与数据存储原理)
        + [3 一条MySql更新语句的执行过程是怎么样的](#3-一条mysql更新语句的执行过程是怎么样的)
* [Ceph](#ceph)
        + [Ceph的整体架构](#ceph的整体架构)
...
* [Qt](#qt)
    - [基础语法](#基础语法)
        + [1 Qt文件流(QTextStream)和数据流(QDataStream)的区别与用途](#1-qt文件流qtextstream和数据流qdatastream的区别与用途)
        + [2 Qt的MVD](#2-qt的mvd)
    - [多线程](#多线程-1)
        + [1 Qt多线程同步的实现方式：](#1-qt多线程同步的实现方式)
        + [2 如何把一个继承于QObject的类转移到Thread里](#2-如何把一个继承于qobject的类转移到thread里)
* [参考](#参考)
