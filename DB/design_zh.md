...
* [2分表分库](#2分表分库)
* [3通用缓存系统](#3通用缓存系统)
    - [3.1缓存系统评判指标](#31缓存系统评判指标)
    - [3.2数据一致性解决方案](#32数据一致性解决方案)
        + [3.2.1方案一 先删缓存再更新数据库](#321方案一-先删缓存再更新数据库)
        + [3.2.2方案二 先删缓存再更新数据库同时引入binlog机制](#322方案二-先删缓存再更新数据库同时引入binlog机制)
        + [3.2.3方案三 在方案二的基础上引入MQ串行化机制](#323方案三-在方案二的基础上引入mq串行化机制)
        + [3.2.4方案四 在方案三的基础上添加标记](#324方案四-在方案三的基础上添加标记)
    - [3.3缓存系统组件](#33缓存系统组件)
    - [3.4缓存系统常见问题](#34缓存系统常见问题)
        + [3.4.1缓存穿透](#341缓存穿透)
        + [3.4.2缓存击穿](#342缓存击穿)
        + [3.4.3 缓存雪崩](#343-缓存雪崩)
* [参考](#参考)
